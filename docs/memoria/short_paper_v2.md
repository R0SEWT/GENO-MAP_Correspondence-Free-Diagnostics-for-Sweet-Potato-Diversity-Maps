# GENO-MAP: Correspondence-Free Quality Diagnostics and Robustness Analysis of Genomic Diversity Maps in Germplasm Collections

## Abstract

Exploring genomic diversity in large germplasm collections is challenged by the high dimensionality of genotyping data and, in practice, by the absence of shared identifiers across independently generated genotyping panels. We present GENO-MAP, a reproducible pipeline that converts DArT and DArTSeq genotype matrices into two-dimensional embeddings and k-nearest neighbor (kNN) similarity graphs for visual exploration and curation. The pipeline chains explicit imputation, hierarchical dimensionality reduction (PCA → UMAP), and kNN graph construction on deterministic PCA spaces, producing structured JSON outputs for interactive visualization. Because the panels studied here use disjoint identifier namespaces (CIP accession codes vs. DArT plate/well coordinates), cross-panel alignment is infeasible without an external manifest. We therefore develop a *correspondence-free* validation framework comprising: (i) per-panel geometry diagnostics (effective rank, kNN reciprocity, connected-component analysis, automatic QA flags), and (ii) robustness curves under controlled perturbations (marker subsampling, MCAR missing injection, imputation strategy comparison). We additionally benchmark a denoising autoencoder against the PCA baseline, finding that the learned bottleneck offers a statistically negligible trustworthiness improvement ($\Delta \leq 0.002$) only above $n/p > 0.20$, while suffering a persistent stability deficit of ~0.45 Jaccard units, confirming PCA as the canonical method for DArT potato genomics. The approach is demonstrated on four sweet potato (*Ipomoea batatas*) datasets (630–5 970 samples × 20 069–62 732 markers), showing that the graph topology degrades gracefully under perturbation: PCA subspace similarity remains above 0.91 even when retaining only 5% of markers, and the pipeline is robust to imputation strategy (mode vs. median Jaccard > 0.91).
...
Se utilizaron tres fuentes de datos de camote (*Ipomoea batatas*) provenientes del repositorio CIP Dataverse. Tras detectar que wild_snp es un duplicado exacto de lowdensity_snp (ADR-0002: mismos 635 samples × 62 732 markers, trustworthiness y estabilidad idénticas), se excluye del análisis, reteniendo cuatro datasets.
...
4. **Grafo kNN**: Sobre las primeras 30 PCs con `sklearn.neighbors.NearestNeighbors`, $k = 15$ y métrica coseno. Las aristas se almacenan como tuplas (source, target, distance).
...
- **Componentes conexas**: Número y tamaño del giant component.
...
**Tabla 3.** Estabilidad inter-seed del grafo kNN. Los grafos PCA son deterministas (Jaccard ≥ 0.989); la variabilidad proviene exclusivamente de UMAP (~0.65–0.71).
...
**Tabla 10.** La estabilidad del AE es sustancialmente inferior: para LowDensity, Jaccard ~0.17 (vs. 0.91 PCA), indicando grafos no reproducibles entre seeds.
...
La frontera de estabilidad (Fig. 10) establece una regla práctica: para DArT genómicos donde $n/p < 0.20$ — lo que cubre el 99% de los datasets de CIP — PCA es óptimo. El crossover a $n^* \approx 4\,000$–$4\,500$ ofrece una ventaja negligible ($\Delta \leq 0.002$) que no justifica la complejidad adicional ni la pérdida de estabilidad.
...
4. El **autoencoder denoising** ofrece una mejora estadísticamente negligible ($\Delta_{\text{trust}} \leq +0.002$) solo para $n/p > 0.20$, con un déficit persistente de estabilidad (~0.45 Jaccard units). Para colecciones genómicas con $n < 1\,000$, PCA es claramente preferible.
...
5. **Un solo cultivo**: Resultados limitados a camote (*Ipomoea batatas*); generalización a otros cultivos andinos no probada.
