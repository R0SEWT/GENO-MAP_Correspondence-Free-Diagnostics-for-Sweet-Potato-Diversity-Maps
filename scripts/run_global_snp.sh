#!/usr/bin/env bash
# Run embedding + kNN graph for sweetpotato global SNP dataset.
set -euo pipefail
...
MAX_SAMPLES="${MAX_SAMPLES:-0}"   # 0 => sin límite
MAX_MARKERS="${MAX_MARKERS:-0}"   # 0 => sin límite
LIMIT_ROWS="${LIMIT_ROWS:-0}"     # 0 => leer todo
SEED="${SEED:-42}"
